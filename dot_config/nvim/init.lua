...
-- Load all spec files from lua/_/specs/*.lua
local function load_specs()
  local specs = {}
  local spec_files = vim.fn.globpath(vim.fn.stdpath('config') .. '/lua/_/specs', '*.lua', false, true)

  for _, file in ipairs(spec_files) do
    local module_name = file:match('lua/_/specs/(.+)%.lua$')
    if module_name then
      table.insert(specs, require('_.specs.' .. module_name))
    end
  end

  return table.combine(unpack(specs))
end
